...
class HumanOutputFormat(OutputFormat):
    def output(self):
        ...
        for output in self.pwstore:
            ...
            if any(keyword in record for keyword in
                   ['twitter', 'discord', 'instagram', 'facebook', 'linkedin', 'reddit', 'snapchat', 'tiktok',
                    'youtube','e621','furaffinity']):
...
def get_profile(basepath: str, interactive: bool, choice: Optional[str] = None, list_profiles: bool = False):
    """
...
def main() -> None:
    ...
    while not working:
        i +=1
        try:
            profile = get_profile(basepath, args.interactive, str(i), args.list)
            working=True
        except:
            mreoww=0
    print(args.choice)
